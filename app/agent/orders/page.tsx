...
const statusConfig: Record<string, { labelEn: string; labelZhCN: string; labelZhTW: string; color: string }> = {
  pending_review: { labelEn: "Pending Review", labelZhCN: "待审核", labelZhTW: "待審核", color: "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400" },
  approved: { labelEn: "Approved", labelZhCN: "已批准", labelZhTW: "已批准", color: "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400" },
...
export default function MyOrdersPage() {
  ...
  const getStatusFilterOptions = (): FilterOption[] => {
    return [
      ...
      { label: lang === "en" ? "Pending Review" : lang === "zh-CN" ? "待审核" : "待審核", value: "pending_review" },
      { label: lang === "en" ? "Approved" : lang === "zh-CN" ? "已批准" : "已批准", value: "approved" },
    ...
    <AgentLayout title={t("my_orders")} subtitle={`${formatNumber(filteredOrders.length)} ${lang === "en" ? "orders" : lang === "zh-CN" ? "个订单" : "個訂單"}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="card p-4">
          ...
          <div className="text-xl font-bold text-amber-600">{formatNumber(filteredOrders.filter((o) => o.status === "pending_review" || o.status === "approved").length)}</div>
        </div>
      </div>
          ...
          <div className="xl:col-span-2">
            <div className="card p-6">
              ...
              <div className="mb-6 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                ...
                <div className="flex items-center gap-2">
                  <div className={`w-3 h-3 rounded-full ${selected.status === "pending_review" || selected.status === "approved" || selected.status === "shipped" || selected.status === "completed" ? "bg-emerald-500" : "bg-slate-300"}`} />
                  <span className="text-xs">{lang === "en" ? "Submitted" : lang === "zh-CN" ? "已提交" : "已提交"}</span>
                  <div className={`flex-1 h-0.5 ${selected.status === "approved" || selected.status === "shipped" || selected.status === "completed" ? "bg-emerald-500" : "bg-slate-200"}`} />
                  <div className={`w-3 h-3 rounded-full ${selected.status === "approved" || selected.status === "shipped" || selected.status === "completed" ? "bg-emerald-500" : "bg-slate-300"}`} />
                  <span className="text-xs">{lang === "en" ? "Approved" : lang === "zh-CN" ? "已批准" : "已批准"}</span>
